<?php
/**
 * File: penguji/form_penilaian_tertutup.php
 * Form penilaian khusus ujian tertutup
 */
...
$sql = "SELECT r.*, m.nama_lengkap, m.nim, m.program_studi, 
        ...
        WHERE r.id_registrasi = ? AND r.jenis_ujian = 'tertutup'";
...
if (!$ujian) {
    die("Data ujian tertutup tidak ditemukan");
}

// Ambil template penilaian tertutup
$template = getTemplatePenilaian('tertutup');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_nilai'])) {
    ...
    if ($stmt->execute()) {
        ...
        $success = "Penilaian ujian tertutup berhasil disimpan! Nilai Total: " . number_format($total_nilai, 2) . " (" . calculateGrade($total_nilai) . ")";
    } else {
...
<div class="form-penilaian-container">
    <div class="header-penilaian">
        <h1>LEMBAR PENILAIAN UJIAN TERTUTUP</h1>
        <h2>PROGRAM STUDI DOKTOR ILMU KOMPUTER</h2>
        <p style="color: #666; font-size: 14px; margin-top: 10px;">
            Penilaian Akhir Disertasi Sebelum Promosi Doktor
        </p>
    </div>
